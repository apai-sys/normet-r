#' Train a model using a specified backend
#'
#' \code{nm_train_model} is a high-level wrapper function that dispatches the training
#' task to a specific backend implementation based on the `backend` parameter.
#' The default backend is 'lightgbm'.
#'
#' @param df Input data frame containing the data to be used for training.
#' @param target The target variable name as a string. Default is "value".
#' @param covariates Independent/explanatory variables used for training the
#'        model. Time variables are opt-in, not automatic -- see
#'        \code{\link{nm_build_model}}'s `covariates` for details.
#' @param model_config Optional list of AutoML configuration overrides.
#'        Examples: `list(max_runtime_secs = 60, include_algos = c("GBM"), nfolds = 5)`.
#' @param backend The modeling framework to use: 'lightgbm' (default) or 'h2o'.
#' @param seed A random seed for reproducibility. Default is 7654321.
#' @param verbose Should the function print progress messages? Default is TRUE.
#' @param cache_dir Character. Directory for on-disk model caching (see
#'        \code{\link{nm_make_cache}}). If NULL (default), caching is
#'        disabled. Repeat calls with the same data (hashed on `covariates`
#'        + `target` + any `set` column), `model_config`, `seed`, and
#'        `backend` are served from disk instead of retraining.
#'
#' @return The trained model object from the specified backend.
#'
#' @examples
#' \donttest{
#' if (requireNamespace("lightgbm", quietly = TRUE)) {
#'   covariates <- c("ws", "wd", "temp", "RH", "blh", "ssrd")
#'   df_prep <- nm_prepare_data(
#'     my1[1:150, c("date", "NO2", covariates)],
#'     target = "NO2", covariates = covariates, verbose = FALSE
#'   )
#'   predictors <- c(covariates, "date_unix", "day_julian", "weekday", "hour")
#'   model <- nm_train_model(
#'     df_prep, target = "value", covariates = predictors,
#'     model_config = list(n_trials = 1, cv_folds = 2, nrounds = 15,
#'                          num_leaves_min = 5, num_leaves_max = 15),
#'     seed = 42, verbose = FALSE
#'   )
#'   nm_extract_features(model)
#' }
#' }
#'
#' @export
nm_train_model <- function(df, target = "value", covariates = NULL, model_config = NULL,
                           backend = "lightgbm", seed = 7654321, verbose = TRUE,
                           cache_dir = NULL) {

  log <- nm_get_logger("model.train")

  backend <- match.arg(backend, choices = c("lightgbm", "h2o"))

  # --- 1. Input Validation ---
  if (missing(covariates) || is.null(covariates) || length(covariates) == 0) {
    stop("`covariates` argument is missing or empty.")
  }

  # Check for NA values in covariates vector
  if (any(is.na(covariates))) {
    stop("`covariates` vector contains NA values. Please check your variable names.")
  }

  # --- 2. Optional Cache Check ---
  cache_key <- NULL
  if (!is.null(cache_dir)) {
    key_cols <- intersect(unique(c(covariates, target, "set")), colnames(df))
    cache_key <- nm_config_hash(
      backend, target, sort(covariates), model_config, seed,
      nm_dataframe_hash(df, cols = key_cols, include_index = FALSE)
    )
    cached <- nm_cache_load(cache_dir, cache_key)
    if (!is.null(cached)) {
      if (verbose) log$info("train_model() cache hit (%s).", cache_key)
      return(cached)
    }
    if (verbose) log$info("train_model() cache miss (%s). Training...", cache_key)
  }

  if (verbose) log$info("Dispatching to backend for training: %s", backend)

  # --- 3. Dispatch to Backend ---
  if (backend == "h2o") {
    model <- nm_train_h2o(
      df = df, target = target, covariates = covariates,
      model_config = model_config, seed = seed, verbose = verbose
    )

  } else if (backend == "lightgbm") {
    model <- nm_train_lgb(
      df = df, target = target, covariates = covariates,
      model_config = model_config, seed = seed, verbose = verbose
    )

  } else {
    err_msg <- paste("Unsupported backend:", backend)
    log$error(err_msg)
    stop(err_msg)
  }

  # --- 4. Optional Cache Save ---
  if (!is.null(cache_dir) && !is.null(cache_key)) {
    nm_cache_save(cache_dir, cache_key, model)
    if (verbose) log$info("train_model() result cached to '%s'.", cache_dir)
  }

  model
}


#' Prepare Data and Train a Model
#'
#' @description
#' `nm_build_model` orchestrates the end-to-end process of data preparation and model training.
#' It separates external covariates from time features, handles data splitting, and dispatches
#' to the specified training backend.
#'
#' @param df The raw input data frame.
#' @param target A string indicating the target column in `df`.
#' @param backend A string for the modeling backend: 'lightgbm' (default) or 'h2o'.
#' @param covariates A character vector of ALL features to be used in the model.
#'        This can include external variables (e.g., "temp") AND specific time variables (e.g., "weekday").
#'        Time variables (`date_unix`/`day_julian`/`weekday`/`hour`) are
#'        opt-in, not automatic: \code{\link{nm_add_date_variables}} always
#'        computes all four internally, but only whichever of them you list
#'        here become model predictors. Include a subset (e.g. only
#'        `weekday`+`hour`) or omit all four entirely to train purely on
#'        meteorology/traffic/other non-temporal predictors --
#'        \code{\link{nm_decompose}} adapts automatically to whatever
#'        subset ends up in the trained model.
#' @param split_method A string for the data splitting strategy. Default is 'random'.
#' @param train_fraction A numeric value for the training fraction of the split. Default is 0.75.
#' @param model_config Optional list of AutoML configuration overrides.
#'        Examples: `list(max_runtime_secs = 60, include_algos = c("GBM"), nfolds = 5)`.
#' @param seed An integer for the random seed. Default is 7654321.
#' @param verbose A logical value to enable logging messages. Default is TRUE.
#' @param cache_dir Character. Directory for on-disk model caching, forwarded
#'        to \code{\link{nm_train_model}}. If NULL (default), disabled.
#'
#' @return A list containing `df_prep` (the processed data) and `model` (the trained model object).
#'
#' @examples
#' \donttest{
#' if (requireNamespace("lightgbm", quietly = TRUE)) {
#'   covariates <- c("ws", "wd", "temp", "RH", "blh", "ssrd")
#'   predictors <- c(covariates, "date_unix", "day_julian", "weekday", "hour")
#'   build <- nm_build_model(
#'     my1[1:150, c("date", "NO2", covariates)],
#'     target = "NO2", covariates = predictors,
#'     model_config = list(n_trials = 1, cv_folds = 2, nrounds = 15,
#'                          num_leaves_min = 5, num_leaves_max = 15),
#'     seed = 42, verbose = FALSE
#'   )
#'   names(build)
#' }
#' }
#'
#' @export
nm_build_model <- function(df,
                           target,
                           backend = "lightgbm",
                           covariates = NULL,
                           split_method = "random",
                           train_fraction = 0.75,
                           model_config = NULL,
                           seed = 7654321,
                           verbose = TRUE,
                           cache_dir = NULL) {

  log <- nm_get_logger("model.build")
  backend <- match.arg(backend, choices = c("lightgbm", "h2o"))

  # 1. Validate Inputs
  if (missing(covariates) || is.null(covariates) || length(covariates) == 0) {
    stop("`covariates` must be provided and cannot be empty.")
  }
  if (!target %in% colnames(df)) {
    stop(sprintf("Target column '%s' not found.", target))
  }

  # --- 2. Smart Separation of Covariates vs Time Features ---
  # We need to know which of the 'covariates' are external variables (that need checking/imputing)
  # and which are time variables (that are auto-generated).
  known_time_vars <- c("date_unix", "day_julian", "weekday", "hour")

  # Covariates for prep = Covariates - Time Vars
  # These are the columns we expect to find in the raw 'df'
  covariates_for_prep <- setdiff(covariates, known_time_vars)

  if (length(covariates_for_prep) == 0) {
    log$warn("No external covariates detected in `covariates`. Model will use time features only.")
  }

  # --- 3. Prepare Data ---
  if (verbose) log$info("Preparing data... (Covariates: %s)", paste(covariates_for_prep, collapse = ", "))

  df_prep <- nm_prepare_data(
    df = df,
    target = target,
    covariates = covariates_for_prep, # Only pass the non-time vars here
    split_method = split_method,
    train_fraction = train_fraction,
    seed = seed,
    verbose = verbose
  )

  # --- 4. Validate Final Covariates ---
  # Ensure all requested covariates (both Met and Time) exist in the prepared data
  missing_preds <- setdiff(covariates, colnames(df_prep))

  if (length(missing_preds) > 0) {
    # If a user asked for "hour" but for some reason it wasn't generated (rare), or a typo in Met var
    stop(sprintf("The following requested covariates are missing from prepared data: %s",
      paste(missing_preds, collapse = ", ")))
  }

  # 5. Train Model
  # We pass the EXACT list of covariates the user requested
  target_col <- if ("value" %in% colnames(df_prep)) "value" else target

  if (verbose) {
    log$info("Training model with %d covariates: %s", length(covariates), paste(covariates, collapse = ", "))
  }

  model <- nm_train_model(
    df = df_prep,
    target = target_col,
    backend = backend,
    covariates = covariates,
    model_config = model_config,
    seed = seed,
    verbose = verbose,
    cache_dir = cache_dir
  )

  if (verbose) log$info("Model training completed successfully.")

  return(list(
    df_prep = df_prep,
    model = model
  ))
}
