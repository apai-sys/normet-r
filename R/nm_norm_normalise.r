...
#'
#' @details
#' Instead of guessing `n_samples` (e.g., 1000), this function runs
#' resampling in batches of `batch_size` and stops when a convergence
#' criterion holds for `stability_streak` consecutive checks. Two criteria
#' are available via `convergence_metric`:
#'
#' * `"series"` (default): stop when the `rse_percentile`-th percentile of
#'   the **per-date relative standard error** of the cumulative mean falls
#'   below `convergence_tol` (default `"3%"`). This targets the actual
...
#'   CLT-based distance-to-limit measure, so the test does not weaken as
#'   the sample count grows.
#' * `"global"` (legacy): stop when the relative batch-to-batch change of
#'   the **global mean** stays below `convergence_tol` (default `"0.5%"`).
#'   Two caveats motivated the new default above: (i) the global mean
#'   averages over all timestamps, suppressing Monte-Carlo noise by roughly
#'   `sqrt(n_dates)`, so it can satisfy a tight tolerance while individual
#'   timestamps are still far from converged; and (ii) the increment of a
#'   cumulative mean shrinks like `1/n_batches` automatically, so the test
#'   gets weaker as sampling proceeds and cannot catch late, slow drift.
#'
...
#' @param convergence_metric `"series"` (default) or `"global"` (legacy).
#'   See Details.
#' @param convergence_tol Numeric fraction or percent string. Stopping
#'   threshold; defaults to `"3%"` for `"series"` and `"0.5%"` for
#'   `"global"`.
#' @param rse_percentile Percentile of the per-date relative-standard-error
#'   distribution tested against `convergence_tol` (only used by
#'   `"series"`). Default 95.
#' @param stability_streak Integer. Consecutive checks required to trigger
#'   stop. Defaults to 3 for `"series"` (the RSE declines smoothly as
#'   `1/sqrt(n)`) and 5 for `"global"` (legacy behaviour).
...
#' @param seed Integer. Random seed for reproducibility. Each batch uses
#'   `seed + cumulative_n` to ensure independent Monte-Carlo resamples.
#'   Default 7654321.
#' @param verbose Logical. Print progress.
#' @param return_history Logical. If TRUE, also return a per-batch
#'   convergence trace (`n`, `metric`, `global_mean`, `stable_count`) --
#'   useful for visualising/auditing the convergence path (e.g. the
#'   "tolerance tunnel" diagnostic plot). Default FALSE.
#' @param ... Additional arguments forwarded to each batch's [nm_normalise()]
#'        call (e.g. `n_cores`, `replace`, `memory_save`,
#'        `feature_names`).
#'
...
#'   per convergence check: `n` (cumulative samples), `metric` (the value
#'   tested against `convergence_tol`), `global_mean`, `stable_count`.
#'
#' @export
nm_normalise_auto <- function(df, model, resample_vars = NULL,
                              resample_df = NULL,
                              convergence_metric = "series",
                              convergence_tol = NULL,
                              rse_percentile = 95,
                              stability_streak = NULL,
                              batch_size = 100,
                              max_samples = 5000,
                              seed = 7654321,
                              verbose = TRUE,
  ...
  convergence_metric <- match.arg(tolower(convergence_metric), c("series", "global"))
  if (is.null(convergence_tol)) {
    convergence_tol <- if (convergence_metric == "series") "3%" else "0.5%"
  }
  ...
  if (is.null(stability_streak)) {
    stability_streak <- if (convergence_metric == "series") 3L else 5L
  }
  ...
  # Accumulator Table: per-date running sum of batch means and of their
  # squares (the latter feeds the series criterion's per-date variance).
  accumulator_dt <- df[, .(date, observed = if ("value" %in% names(df)) value else get(grep("obs|value", names(df), value = T)[1]))]
  accumulator_dt[, `:=`(sum_bm = 0, sum_bm2 = 0)]
  data.table::setkey(accumulator_dt, date)

  if (verbose) {
    message(sprintf("Starting Auto-Normalisation (Metric: %s, Tol: %.2f%%, Batch: %d)",
                    convergence_metric, convergence_tol * 100, batch_size))
  ...
  while (total_n < max_samples) {
    ...
    # batch_res$normalised is the mean of `batch_size` samples -- one batch
    # mean per date per batch.
    accumulator_dt[batch_res, `:=`(
      sum_bm  = sum_bm + i.normalised,
      sum_bm2 = sum_bm2 + i.normalised^2
    ), on = "date"]
    ...
    if (n_batches >= 2) {
      cum_mean <- accumulator_dt$sum_bm / n_batches
      current_global_mean <- mean(cum_mean, na.rm = TRUE)

      if (convergence_metric == "series") {
        # Per-date relative standard error of the cumulative mean:
        # SE_t = sd(batch means at t)/sqrt(n_batches); RSE_t = SE_t/|mean_t|.
        var_bm <- (accumulator_dt$sum_bm2 - accumulator_dt$sum_bm^2 / n_batches) / (n_batches - 1)
        se <- sqrt(pmax(var_bm, 0) / n_batches)
        denom <- abs(cum_mean)
        ok <- is.finite(denom) & denom > 1e-12 & is.finite(se)
        metric_value <- if (any(ok)) {
          as.numeric(stats::quantile(se[ok] / denom[ok], rse_percentile / 100, names = FALSE))
        } else {
          Inf
        }
      } else {
        metric_value <- if (prev_global_mean != 0) {
          abs((current_global_mean - prev_global_mean) / prev_global_mean)
        } else {
          Inf
        }
      }

      if (is.finite(metric_value) && metric_value < convergence_tol) {
      ...
      if (return_history) {
        history_list[[length(history_list) + 1]] <- data.frame(
          n = total_n, metric = metric_value,
          global_mean = current_global_mean, stable_count = stable_count)
      }

      if (stable_count >= stability_streak) {
        if (verbose) {
          message(sprintf("\n\n--- Convergence Reached! ---"))
          message(sprintf("Stopped at n = %d | Final Global Mean: %.4f | Metric: %.5f%%",
            total_n, current_global_mean, metric_value * 100))
        }
        converged <- TRUE
        break
      }
      ...
      prev_global_mean <- mean(accumulator_dt$sum_bm / n_batches, na.rm = TRUE)
    }
  ...
  if (verbose) close(pb)
  if (!converged) warning("Reached max_samples limit without strict convergence.")

  # --- 3. Finalize ---
  accumulator_dt[, normalised := sum_bm / n_batches]
